  ...
  """
    ...
    for key in _VOCAB_FEATURE_KEYS:
        ...
        outputs[_transformed_name(key)] = tft.compute_and_apply_vocabulary(
            _fill_in_missing(inputs[key]), top_k=_VOCAB_SIZE, num_oov_buckets=_OOV_SIZE
        )
...
def _eval_input_receiver_fn(transform_output, schema):
  ...
  """
    ...
    _, transformed_features = saved_transform_io.partially_apply_saved_transform(
        os.path.join(transform_output, transform_fn_io.TRANSFORM_FN_DIR), features
    )
...
def _input_fn(filenames, transform_output, batch_size=200):
  ...
  """
    metadata_dir = os.path.join(
        transform_output, transform_fn_io.TRANSFORMED_METADATA_DIR
    )
...
def trainer_fn(hparams, schema):
  ...
  """
    ...
    train_input_fn = lambda: _input_fn(  # pylint: disable=g-long-lambda
        hparams.train_files, hparams.transform_output, batch_size=train_batch_size
    )

    eval_input_fn = lambda: _input_fn(  # pylint: disable=g-long-lambda
        hparams.eval_files, hparams.transform_output, batch_size=eval_batch_size
    )
